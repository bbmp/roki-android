...
public class MobileRecipeCookTaskService extends RecipeCookTaskService {

    private MobileRecipeCookTaskService(ArrayList<CookStep> cookSteps, ICookingCallBack callBack) {
        ...
        public static IAbsRecipeCookInterface newCookTaskInstance(ArrayList<CookStep> cookSteps, ICookingCallBack cb) throws Exception{
            if (getCookTaskLIst().size() < getMaxCookinstance()) {
                synchronized (CookTaskFactory.class) {//进入锁后，需要再次判断 排除new对象影响
                    if (getCookTaskLIst().size() < getMaxCookinstance())
                        return new MobileRecipeCookTaskService(cookSteps, cb);
                    else
                        return null;
                }
            } else
