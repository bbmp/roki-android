...
public class PluginADPlay extends PluginOverlay implements DetailMessage {
	...
	private void init(Context context) {
		...
		// 返回按钮
		backImgView = containerView.findViewById(R.id.header_back);
		...
		// 广告倒计时
		mCountUpdateTextView = containerView
				.findViewById(R.id.my_ad_count);

		// 会员免广告
		mVipNoAdBtn = containerView.findViewById(R.id.my_ad_remove);
		...
		// 全屏/半屏按钮
		mSmallFullImgView = containerView
	...
	private void initPauseAdvView() {
		...
		imgAdvContentImgView = findViewById(R.id.adv_img_imgview);
		imgAdvCloseImgView = findViewById(R.id.adv_img_close_imgview);
	...
	public void onPluginAdded() {
		...
		if (mBasePlayerManager.isImageADShowing) {
			...
			Glide.with(mActivity).load(rs).listener(new RequestListener<Drawable>() {
				@Override
	...
	public boolean isCountUpdateVisible() {
		if (mCountUpdateTextView != null) {
			return mCountUpdateTextView.getVisibility() == View.VISIBLE;
		}
		return false;
	}
