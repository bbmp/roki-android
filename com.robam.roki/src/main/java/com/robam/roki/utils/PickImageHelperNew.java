...
public class PickImageHelperNew {
    ...
    public PickImageHelperNew(Activity act, PickCallbackTwo callback) {
        this.act = act;
    ...
    public void onEvent(ActivityResultOnPageEvent event) {
        ...
        switch (requestCode) {
            ...
            case PHOTO_REQUEST_CUT:// 3
                ...
                EventUtils.unregist(PickImageHelperNew.this);
                setPicToView(data);
                break;
            default:
                break;
    ...
    public void showPickDialog(String title) {
        EventUtils.regist(PickImageHelperNew.this);
...
//                    第一个参数Context，第二个参数随便写个字符串，第三个参数就是我们刚刚创建的File对象
    ...
    */
    ...
    private void crop(Uri uri) {
        String currentDate = DateUtil.getCurrentDate(DateUtil.PATTERN_NO_BLANK);
        File CropPhoto = new File(act.getExternalCacheDir(), currentDate +"_crop_image.jpg");
        try {
            if (CropPhoto.exists()) {
                CropPhoto.delete();
            }
    ...
    protected void setPicToView(Intent data) {
        ...
        if (data != null) {
            try {
                ...
                if (bitmap != null) {
                    callback.onPickComplete(new File(new URI(cropImageUri.toString())));
                }
