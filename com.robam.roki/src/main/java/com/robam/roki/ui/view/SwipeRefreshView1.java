...
public class SwipeRefreshView1 extends SwipeRefreshLayout {
    ...
    public SwipeRefreshView1(Context context, AttributeSet attrs) {
        super(context, attrs);
        // 填充底部加载布局
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mListView == null) {
            ...
            if (getChildCount() > 0) {
                // 判断第一个孩子是不是ListView
                if (getChildAt(0) instanceof GridViewWithHeaderAndFooter) {
                    // 创建ListView对象
    ...
    private boolean canTouchMode = true;

    public void setCanTouchMode(boolean canTouchMode) {
        this.canTouchMode = canTouchMode;
    }
