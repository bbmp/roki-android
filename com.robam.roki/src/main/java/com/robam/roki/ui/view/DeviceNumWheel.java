 ...
 */
public class DeviceNumWheel extends View {
    ...
    public DeviceNumWheel(Context context, AttributeSet attrs,
                          int defStyle) {
    ...
    public DeviceNumWheel(Context context) {
        super(context);
        initData();
    }
...
//            setMeasuredDimension(getWidth(), itemNumber * unitHeight - (itemNumber / 2) * (itemNumber / 2 + 1) * heightDiv);
    ...
    private void actionMove(int move) {
       ...
       /* List<ItemObject> templist = new ArrayList<ItemObject>();
        for (ItemObject item : itemList) {
           templist.add(item);
        }
