...
public class DeviceOven028AssistWheel extends View {
     ...
     * 选择的内容
     */
    private List<DeviceOven028AssistWheel.ItemObject> itemList = Lists.newArrayList();
    /**
     * 设置数据
     */
     ...
     * 选择监听
     */
    private DeviceOven028AssistWheel.OnSelectListener onSelectListener;
    /**
     * 是否可用
     */
    ...
    public DeviceOven028AssistWheel(Context context, AttributeSet attrs,
                                  int defStyle) {
    ...
    public DeviceOven028AssistWheel(Context context) {
        super(context);
        initData();
    }
    ...
    private synchronized void drawList(Canvas canvas) {
        if (isClearing)
            return;
        try {
            for (DeviceOven028AssistWheel.ItemObject itemObject : itemList) {
    ...
    private void noEmpty() {
        if (!noEmpty)
            return;
        for (DeviceOven028AssistWheel.ItemObject item : itemList) {
            if (item.isSelected())
                return;
        }
    ...
    private void initData() {
        ...
        for (int i = 0; i < dataList.size(); i++) {
            DeviceOven028AssistWheel.ItemObject itmItemObject = new DeviceOven028AssistWheel.ItemObject();
            itmItemObject.id = i;
    ...
    private int chooseRes(int i, boolean isSelected) {
        ...
        switch (i) {
            case 0:
                ans = isSelected ? R.mipmap.ic_oven028_recently_unfreeze: R.mipmap.ic_oven028_recently_unfreeze;
                break;
            case 1:
                ans = isSelected ? R.mipmap.ic_oven028_recently_fa : R.mipmap.ic_oven028_recently_fa;
                break;
            case 2:
                ans = isSelected ? R.mipmap.ic_oven028_recently_shajun : R.mipmap.ic_oven028_recently_shajun;
                break;
        }
        return ans;
    }
    ...
    public void setDefault(int index) {
        ...
        DeviceOven028AssistWheel.ItemObject item = itemList.get(index);
        onEndSelect(item);
    }
     ...
     */
    public void setOnSelectListener(DeviceOven028AssistWheel.OnSelectListener onSelectListener) {
    ...
    private class ItemObject {
        ...
        public void drawSelf(Canvas canvas) {
            ...
            canvas.drawText(itemText, x + 40, y + 10+move+bitmap.getWidth()+20,
                    textPaint);
    ...
    void onEndSelect(DeviceOven028AssistWheel.ItemObject item) {
        if (onSelectListener != null) {
            onSelectListener.endSelect(item.id, item.tag);
        }
    }

    void onSelecting(DeviceOven028AssistWheel.ItemObject item) {
        if (onSelectListener != null) {
