 ...
 * 专业模式中模式选择
 */
public class RvModeAdapter extends BaseQuickAdapter<DeviceConfigurationFunctions, BaseViewHolder> {
    ...
    protected void convert(@NotNull BaseViewHolder holder, DeviceConfigurationFunctions item) {
        ...
        if (item != null){
            ...
            if (holder.getLayoutPosition() == selectPosition){
                mDrawable=ImageSelectUtil.getResIdSelect(item.functionCode);
...
//                ImageUtils.displayImage(item.backgroundImgH, (ImageView) holder.getView(R.id.iv_model));
                ...
                mDrawable=ImageSelectUtil.getResIdUnSelect(item.functionCode);
