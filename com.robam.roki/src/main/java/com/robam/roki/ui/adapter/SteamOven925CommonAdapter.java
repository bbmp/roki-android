 ...
 */
public class SteamOven925CommonAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    /**
     * 0位置的数据
     */
    ...
    public SteamOven925CommonAdapter(Context context, List<DeviceConfigurationFunctions> mainList, List<DeviceConfigurationFunctions> otherList) {
        this.cx = context;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == GridSHOW) {
            return new SteamOvenComViewHolder(mLayoutInflater.inflate(R.layout.steamoven_grid_mode, null));
        } else if (viewType == GridMORE) {
            return new SteamOvenMoreViewHolder(mLayoutInflater.inflate(R.layout.steamoven_more_mode_show, null));
        } else if (viewType == ITEMSHOW) {//item_otherfunc_page
            return new SteamOvenItemViewHolder(mLayoutInflater.inflate(R.layout.oven_recycleview_show, null));
        }
        return null;
    }

    SteamOvenMoreViewHolder moreViewHolder;

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (getItemViewType(position) == GridSHOW) {
            SteamOvenComViewHolder steamOvenComViewHolder = (SteamOvenComViewHolder) holder;
            steamOvenComViewHolder.setData(mainList);
        } else if (getItemViewType(position) == GridMORE) {
            moreViewHolder = (SteamOvenMoreViewHolder) holder;
        } else if (getItemViewType(position) == ITEMSHOW) {
            SteamOvenItemViewHolder steamOvenItemViewHolder = (SteamOvenItemViewHolder) holder;
    ...
    class SteamOvenComViewHolder extends RecyclerView.ViewHolder {
        ...
        private void setData(final List<DeviceConfigurationFunctions> mainList) {
            ...
            gv.setAdapter(new SteamOvenComViewHolder.MyGridViewAdapter());
        ...
        class MyGridViewAdapter extends BaseAdapter {
            ...
            public View getView(int position, View convertView, ViewGroup viewGroup) {
                try {
                    SteamOvenComViewHolder.ViewHolder holder = null;
                    if (convertView == null) {
                        //没缓存
                        convertView = mLayoutInflater.inflate(R.layout.oven_item_show, null);
                        holder = new SteamOvenComViewHolder.ViewHolder();
                        ...
                        //有缓存
                        holder = (SteamOvenComViewHolder.ViewHolder) convertView.getTag();
                    }
                    //绑定viewholder数据
    ...
    class SteamOvenItemViewHolder extends RecyclerView.ViewHolder {
        ...
        private void setData(List<DeviceConfigurationFunctions> otherList) {
            ...
            recycle.setAdapter(new SteamOvenItemViewHolder.ItemshowAdapter());
        }

        class ItemshowAdapter extends RecyclerView.Adapter<SteamOvenItemViewHolder.ItemshowAdapter.ItemViewHolder> {

            @Override
            public SteamOvenItemViewHolder.ItemshowAdapter.ItemViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
                return new SteamOvenItemViewHolder.ItemshowAdapter.ItemViewHolder(mLayoutInflater.inflate(R.layout.item_otherfunc925_page, null));
            }

            @Override
            public void onBindViewHolder(SteamOvenItemViewHolder.ItemshowAdapter.ItemViewHolder holder, final int position) {
                SteamOvenItemViewHolder.ItemshowAdapter.ItemViewHolder itemViewHolder = holder;
...
//                if(position%2==0){
//                    holder.mTvCurveTips.setVisibility(View.GONE);
//                }
            }
            ...
            class ItemViewHolder extends RecyclerView.ViewHolder {
                ...
                public ItemViewHolder(View itemView) {
                    ...
                    mTvCurveTips = (TextView) itemView.findViewById(R.id.tv_curve_tips);
                }
            }
        }
