...
public class DeviceFanPage extends HeadPage {
    ...
    protected View onCreateContentView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        ...
        View view = layoutInflater.inflate(R.layout.page_device_fan, viewGroup, false);
...
//        } else {
//        }
        /*if (DeviceTypeManager.getInstance().isInDeviceType(fan.getGuid(), IRokiFamily.R8700)) {
            ctrView = new FanCtr8700View(cx);
        } else*/
       /* if (DeviceTypeManager.getInstance().isInDeviceType(fan.getGuid(), IRokiFamily.R9700)
                ||DeviceTypeManager.getInstance().isInDeviceType(fan.getGuid(), IRokiFamily.R8230)) {
            ctrView = new FanCtr9700View(cx);*/
       // } else
        if (DeviceTypeManager.getInstance().isInDeviceType(fan.getGuid(),IRokiFamily.R8229)){
            ctrView = new FanCtr8229View(cx);
        } else if (DeviceTypeManager.getInstance().isInDeviceType(fan.getGuid(), IRokiFamily.R66A2)){
           ctrView = new FanCtr66A2View(cx);
        }else if (DeviceTypeManager.getInstance().isInDeviceType(fan.getGuid(), IRokiFamily.R5610)){
