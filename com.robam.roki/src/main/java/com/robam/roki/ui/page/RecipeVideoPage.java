...
public class RecipeVideoPage extends AbsListViewPage<CookingKnowledge> {
    ...
    class LiveListAdapter extends ListAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (contentType == 0) {
                ImageUtils.displayImage(live.pictureCoverUrl, viewHolder.imgView);
                if (!StringUtils.isNullOrEmpty(live.pictureCoverUrl))
                    ImageUtils.displayImage(live.pictureCoverUrl, viewHolder.imgView, options.showImageOnLoading(R.mipmap.img_default).build());
                else
                    viewHolder.imgView.setImageDrawable(r.getDrawable(R.drawable.shape_recipe_void_img_bg));
                viewHolder.title.setText(live.title);
            } else {
                ImageUtils.displayImage(live.videoCoverUrl, viewHolder.imgView);
                if (!StringUtils.isNullOrEmpty(live.videoCoverUrl))
                    ImageUtils.displayImage(live.videoCoverUrl, viewHolder.imgView, options.showImageOnLoading(R.mipmap.img_default).build());
                else
                    viewHolder.imgView.setImageDrawable(r.getDrawable(R.drawable.shape_recipe_void_img_bg));
                viewHolder.title.setText(live.title);
            }
