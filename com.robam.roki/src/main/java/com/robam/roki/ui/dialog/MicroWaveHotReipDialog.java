...
public class MicroWaveHotReipDialog extends AbsDialog {
    ...
    public MicroWaveHotReipDialog(Context context, List<Integer> tempUp, List<Integer> tempDwon,
                                  List<Integer> time, List<String> liststr, int defaultTemp,
                                  int defaultdwon, int defaultTime, int stepC, int defaultValue, int min) {
        ...
        // this.default2 = defaultdwon;
        this.min = min;
        if (tempUp.size() > 0) {
            this.max = tempUp.get(tempUp.size() - 1);
        }
        this.tempDown = SteamOvenDatas.createExpDownDatas(tempUp.get(default1), stepC, defaultValue, max, min);
        init();
    }
    ...
    protected int getViewResId() {
        return R.layout.abs_module_hot_reip_dialog;
    }
    ...
    private void init() {
        ...
        wv1.setOnItemSelectedListener(new WheelView.OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                tempDown = SteamOvenDatas.createExpDownDatas(tempUp.get(index), stepC, defaultValue, max, min);
                wv2.setAdapter(new TimeAdapter<Integer>(tempDown, str2));
            }
        });
    ...
    public void show(MicroWaveHotReipDialog absSetting) {
