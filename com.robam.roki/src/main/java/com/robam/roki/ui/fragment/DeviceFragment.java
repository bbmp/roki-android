...
import com.robam.roki.ui.view.DeviceNewItemView;
 ...
 * desc   : 智能 Fragment
 */
public final class DeviceFragment extends TitleBarFragment<HomeActivity>
        implements UIListeners.IRefresh, OnRequestListener {

    private RelativeLayout rel_homedevice;
    private LinearLayout mLlTitle;
    private ImageView icDeviceScan;
    private ImageView icDeviceAdd;
    private ImageView fragImageMessage;
    private TextView fragTxtNotificationNumber;
    private TextView mTvKitchenKnowledge;
    private RecyclerView mRecyclerView;
    private TextView mTvDeviceIntellectualProducts;
    ...
    private HomeDeviceView.UIRefreshBroadcastReceiver receiver;
    //定义界面刷新常量
...
//                    mDeviceNewItemView.setImageDeviceOfflinePrompt(R.mipmap.img_offline_prompt);
...
//                    mDeviceNewItemView.setImageDeviceOfflinePromptNull();
                ...
                mDeviceNewItemView.setImageDeviceOfflinePrompt(event.isConnected);
                break;
            }
        }
    ...
    protected void initView() {
        ...
        rel_homedevice = findViewById(R.id.rel_homedevice);
        mLlTitle = findViewById(R.id.title);
        icDeviceScan = findViewById(R.id.ic_device_scan);
        icDeviceAdd = findViewById(R.id.ic_device_add);
        fragImageMessage = findViewById(R.id.frag_image_message);
        fragTxtNotificationNumber = findViewById(R.id.frag_txt_notification_number);
        mTvKitchenKnowledge = findViewById(R.id.tv_kitchen_knowledge);
        mRecyclerView = findViewById(R.id.recyclerView);
        mTvDeviceIntellectualProducts = findViewById(R.id.tv_device_intellectual_products);
        mGif = findViewById(R.id.gif);
        mIvAdd = findViewById(R.id.iv_add);
        pullRefreshScrollview = findViewById(R.id.pull_refresh_scrollview);
...
//                    LoginUtil.checkWhetherLogin(getContext(), PageKey.UserLogin);
                    LoginUtil.checkWhetherLogin2(getContext(), getAttachActivity());
                } else if (!isLog && !wifi) {
//                    LoginUtil.checkWhetherLogin(getContext(), PageKey.UserLogin);
                    LoginUtil.checkWhetherLogin2(getContext(), getAttachActivity());
                }
    ...
    void setTitleBar() {
        findViewById(R.id.ic_device_add).setOnClickListener(v -> {
            boolean wifi = NetWorkStateUtils.isWifi(getContext());
            boolean isLog = Plat.accountService.isLogon();
            if (!isLog) {
                startLogin();
        ...
        findViewById(R.id.ic_device_scan).setOnClickListener(new View.OnClickListener() {
...
//                LogUtils.i("20170330", "Robic:" + finaldeviceList.get(i).getDt());
            ...
            if (mCookingKnowledges != null && mCookingKnowledges.size() > 0) {
                mIntellkitchenAdapter = new IntellkitchenAdapter(cx, mCookingKnowledges
                ...
                mRecyclerView.setLayoutManager(new LinearLayoutManager(cx));
            }
    ...
    private void AbsSteamBakeState(String commandApi) {
        ...
        for (int i = 0; i < finaldeviceList.size(); i++) {
            if (finaldeviceList.get(i) instanceof AbsSteameOvenOne) {
                ...
                short powerStatus = absSteameOvenOne.powerOnStatus;
...
//    LogUtils.i("");
        ...
        if (absSteameOvenOne.powerOnStatus == SteamOvenOnePowerStatus.Off) {
            LogUtils.i("201910121111", "off" + ":::::    " + absSteameOvenOne.powerOnStatus);
            ...
            if (paramObject != null) {
                if (paramObject instanceof MessageUserUnreadBean) {
                    ...
                    if (!Plat.accountService.isLogon() || ((MessageUserUnreadBean) paramObject).getPayload().getTotalUnReadCount() == 0) {
                        findViewById(R.id.frag_txt_notification_number).setVisibility(GONE);
                    } else {
...
//                            ((TextView) findViewById(R.id.frag_txt_notification_number)).setTextSize(getResources().getDimension(R.dimen.sp_10));
                        ...
                        findViewById(R.id.frag_txt_notification_number).setVisibility(VISIBLE);
                    }
                }
            }
        }
