...
public class DeviceOvenCommonModeWheel extends View {
    /**
     * 选择监听
     */
     ...
     * 选择的内容
     */
    private List<DeviceOvenCommonModeWheel.ItemObject> itemList = Lists.newArrayList();
    /**
     * 设置数据
     */
     ...
     * 选择监听
     */
    private DeviceOvenCommonModeWheel.OnSelectListener onSelectListener;
    /**
     * 是否可用
     */
    ...
    public DeviceOvenCommonModeWheel(Context context, AttributeSet attrs,
                                  int defStyle) {
    ...
    public DeviceOvenCommonModeWheel(Context context) {
        super(context);
        initData();
    }
    ...
    private synchronized void drawList(Canvas canvas) {
        if (isClearing)
            return;
        try {
            for (DeviceOvenCommonModeWheel.ItemObject itemObject : itemList) {
    ...
    private void noEmpty() {
        if (!noEmpty)
            return;
        for (DeviceOvenCommonModeWheel.ItemObject item : itemList) {
            if (item.isSelected())
                return;
        }
    ...
    private void initData() {
      ...
      /*  List<String> list = Lists.newArrayList();
        ...
        list.add("强烤烧");*/
      //  this.dataList = list;
        LogUtils.i("20180818","dataList::"+dataList.size());
        for (int i = 0; i < dataList.size(); i++) {
            DeviceOvenCommonModeWheel.ItemObject itmItemObject = new DeviceOvenCommonModeWheel.ItemObject();
            itmItemObject.id = i;
    ...
    public void setList(List<String> listDate){
        LogUtils.i("20170818","listdate::"+listDate.size());
        this.dataList = listDate;
    }
    ...
    private int chooseRes2(int i, boolean isSelected) {
        LogUtils.i("20170818","map::"+mapImg.size());
        return mapImg.get(i);
    }
    ...
    public void setDefault(int index) {
        ...
        DeviceOvenCommonModeWheel.ItemObject item = itemList.get(index);
        onEndSelect(item);
    }
     ...
     */
    public void setOnSelectListener(DeviceOvenCommonModeWheel.OnSelectListener onSelectListener) {
    ...
    void onEndSelect(DeviceOvenCommonModeWheel.ItemObject item) {
        if (onSelectListener != null) {
            onSelectListener.endSelect(item.id, item.tag);
        }
    }

    void onSelecting(DeviceOvenCommonModeWheel.ItemObject item) {
        if (onSelectListener != null) {
