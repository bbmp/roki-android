...
public class FilterStateOneView extends FrameLayout {
    ...
    public FilterStateOneView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context, attrs);
    }

    public FilterStateOneView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public FilterStateOneView(Context context) {
        super(context);
        init(context, null);
    }
    ...
    private void init(Context cx, AttributeSet attrs) {
        View view = LayoutInflater.from(cx).inflate(R.layout.view_filter_one, this, true);
        if (!view.isInEditMode()) {
            ButterKnife.inject(this, view);
        }
    }

    public void setPercentData(int percent_pp, int percent_cto, int percent_ro1, int percent_ro2,
                               JSONArray params) {
        if (percent_pp <= 10) {
            mIvFilterOne.setImageResource(R.mipmap.ic_bord_orang);
            mTvPercentPp.setTextColor(getResources().getColor(R.color.c66));
            mTvNumber.setTextColor(getResources().getColor(R.color.c66));
        } else {
            mIvFilterOne.setImageResource(R.mipmap.ic_bord_blue);
            mTvPercentPp.setTextColor(getResources().getColor(R.color.c67));
        ...
        try {
            JSONObject jsonOne = (JSONObject) params.get(0);
            JSONObject oneStep = (JSONObject) jsonOne.get("1");
