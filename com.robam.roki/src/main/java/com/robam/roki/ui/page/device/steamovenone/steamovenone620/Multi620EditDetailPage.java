...
import com.robam.roki.ui.adapter3.Rv620StepAdapter;
 ...
 * 620离线菜谱详情
 */
public class Multi620EditDetailPage extends MyBasePage<MainActivity> {
    ...
    RecyclerView rv620Step;
    /**
     * 步骤adapter
     */
    private Rv620StepAdapter rv620StepAdapter;
    /**
     * 携带菜谱id
     */
    ...
    protected int getLayoutId() {
        return R.layout.page_multi_620_recipe_edit;
    }
    ...
    protected void initView() {
       getTitleBar().setOnTitleBarListener(this);
        ...
        rv620Step.setLayoutManager(new LinearLayoutManager(cx, RecyclerView.VERTICAL, false));
        rv620StepAdapter = new Rv620StepAdapter();
        rv620Step.setAdapter(rv620StepAdapter);
        View footViewAdd = getLayoutInflater().inflate(R.layout.item_d620_add_step, new FrameLayout(cx), false);
        rv620StepAdapter.addFooterView(footViewAdd);

        rv620StepAdapter.addChildClickViewIds(R.id.stb_step , R.id.tv_del);
        rv620StepAdapter.addChildLongClickViewIds(R.id.stb_step);
        rv620StepAdapter.setOnItemChildClickListener(new OnItemChildClickListener() {
            @Override
            public void onItemChildClick(@NonNull @NotNull BaseQuickAdapter baseQuickAdapter, @NonNull @NotNull View view, int i) {
                if (view.getId() == R.id.stb_step){
//                    rv610StepAdapter.setSelectPosition(i);
                    selectStep(i);
                }else if (view.getId() == R.id.tv_del){
                    if (recipeId != -1){
                        LitePal.delete(RecipeStepBean.class , rv620StepAdapter.getItem(i).getId());
                        rv620StepAdapter.removeAt(i);
                    }else {
                        rv620StepAdapter.removeAt(i);
                    }
                    rv620StepAdapter.setSelectPosition(-1);
                }
            }
        });

        rv620StepAdapter.setOnItemChildLongClickListener(new OnItemChildLongClickListener() {
            @Override
            public boolean onItemChildLongClick(@NonNull @NotNull BaseQuickAdapter baseQuickAdapter, @NonNull @NotNull View view, int i) {
                if (view.getId() == R.id.stb_step){
                    rv620StepAdapter.setSelectPosition(i);
                }
...
//        });
        footViewAdd.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (rv620StepAdapter.getItemCount() >= 4){
    ...
    protected void initData() {
        ...
        if (bd != null) {
            ...
            if (recipeId != -1 ){
                ...
                rv620StepAdapter.addData(firstRecipe.getRecipeStepList());
...
//                recipeName.setEnabled(false);
    ...
    public void onClick(View view) {
        if (view == btnAutomatic){
            ...
            if (rv620StepAdapter.getData()==null){
                ToastUtils.show("多段添加不能少于2步");
                return;
            }
            //多段是否含有需要用水箱的

            if (rv620StepAdapter.getData().size()<=1){
                ToastUtils.show("多段添加不能少于2步");
                return;
            }
            ...
            RecipeBean d620Recipe = new RecipeBean(recipeId ,recipeName.getText().toString(), Plat.accountService.getCurrentUserId(), "D620", System.currentTimeMillis());
            d620Recipe.save();
            RecipeBean recipeBean = LitePal.where("recipe_id = ?", recipeId).findFirst(RecipeBean.class);
            List<RecipeStepBean> datas = rv620StepAdapter.getData();
...
//                    recipeStepBean.setRecipe_id(firstRecipe);
                    if (position != -1){
                        recipeStepBean.update(rv620StepAdapter.getItem(position).getId());
//                        rv610StepAdapter.setData(position , recipeStepBean);
                        firstRecipe = LitePal.where("id = ?", recipeId + "").findFirst(RecipeBean.class);
                        rv620StepAdapter.setNewInstance(firstRecipe.getRecipeStepList());
                    }else {
                        recipeStepBean.setRecipebean(firstRecipe);
                        recipeStepBean.save();
                        rv620StepAdapter.addData(recipeStepBean);
                    }
                    firstRecipe = LitePal.where("id = ?", recipeId + "").findFirst(RecipeBean.class);
                    multiInfo.setText( + firstRecipe.getRecipeStepTimes() + "min");
                }else {
                    if (position != -1){
                        rv620StepAdapter.setData(position , recipeStepBean);
                    }else {
                        rv620StepAdapter.addData(recipeStepBean);
                    }
                }
...
//                });
    ...
    public void onDestroyView() {
        ...
        EventUtils.postEvent(new PageBackEvent("Multi620EditDetailPage"));
