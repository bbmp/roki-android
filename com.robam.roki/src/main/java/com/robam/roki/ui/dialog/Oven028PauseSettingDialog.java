...
//    TextView oven026_title;
    ...
    public Oven028PauseSettingDialog(Context context, short autoModel, short model, PickListener callback) {
...
//                    oven026_title.setText(new String("快热"));
                ...
                case OvenMode.FENGBEIKAO:
                    initTempUp(getList2(OvenMode.FENGBEIKAO), 150);
                    initMin(init1to120_min(), 29);
//                    oven026_title.setText(new String("风焙烤"));
                    break;
                case OvenMode.BEIKAO:
                    initTempUp(getList2(OvenMode.BEIKAO), 110);
                    initMin(init1to120_min(), 29);
...
//                    oven026_title.setText(new String("底加热"));
                ...
                case OvenMode.FENGSHANKAO:
                    initTempUp(getList2(OvenMode.FENGSHANKAO), 170);
                    initMin(init1to120_min(), 29);
...
//                    oven026_title.setText(new String("烤烧"));
                ...
                case OvenMode.QIANGKAOSHAO:
                    initTempUp(getList2(OvenMode.QIANGKAOSHAO), 130);
                    initMin(init1to120_min(), 29);
...
//                    oven026_title.setText(new String("EXP"));
                ...
                case OvenMode.KUAISUYURE:
                    initTempUp(getList2(OvenMode.KUAISUYURE), 130);
                    initMin(initDefault90(), 0);
                    break;
                case OvenMode.JIANKAO:
                    initTempUp(getList2(OvenMode.JIANKAO), 100);
                    initMin(init1to120_min(), 29);
                    break;
                case OvenMode.GUOSHUHONGGAN:
                    initTempUp(getList2(OvenMode.GUOSHUHONGGAN), 10);
                    initMin(init1to120_min(), 29);
                    break;
                default:
                    break;
...
//                + oven026_pausesetting_downtemp.getSelectedTag() + "/" + oven026_pausesetting_min.getSelectedTag(), Toast.LENGTH_SHORT);
   ...
   List<Integer> initDefault90(){
    ...
    protected List<Integer> getList2(int index) {
        ...
        }else if (OvenMode.KUAISUYURE==index||OvenMode.JIANKAO == index){
            for (int i = 50; i <=250 ; i++) {
                list.add(i);
            }
        }else if (OvenMode.GUOSHUHONGGAN==index){
            for (int i = 50; i <=80 ; i++) {
                list.add(i);
            }
        }
