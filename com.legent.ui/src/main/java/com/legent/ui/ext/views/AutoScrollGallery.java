...
public class AutoScrollGallery extends Gallery {
    ...
    public AutoScrollGallery(Context paramContext) {
        super(paramContext);
        init();
    }

    public AutoScrollGallery(Context paramContext, AttributeSet paramAttributeSet) {
    ...
    public void scrollOnce() {
        ...
        if (nextItem < 0) {
            if (isCycle) {
                setSelection(totalCount - 1, isBorderAnimation);
            }
        } else if (nextItem == totalCount) {
            if (isCycle) {
                setSelection(0, isBorderAnimation);
            }
        } else {
            setSelection(nextItem, true);
        }
    }
        ...
        if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT || slideBorderMode == SLIDE_BORDER_MODE_CYCLE) {
            ...
            if ((currentItem == 0 && downX <= touchX) || (currentItem == pageCount - 1 && downX >= touchX)) {
                if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT) {
                    ...
                    if (pageCount > 1) {
                        setSelection(pageCount - currentItem - 1, isBorderAnimation);
                    }
