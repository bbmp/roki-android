...
public class StatusBarCompat {
    ...
    public static void updateStatusTextColor(Activity activity) {
        // 修改状态栏字体颜色
        boolean useDarkStatusBar = activity.getResources().getBoolean(R.bool.use_dark_status);
        int resId = SkinCompatResources.getInstance().getTargetResId(activity, R.bool.use_dark_status);
        if (resId != 0) {
            useDarkStatusBar = SkinCompatResources.getInstance().getSkinResources().getBoolean(resId);
        }
        if (useDarkStatusBar) {
            SkinStatusBarUtils.setStatusBarDarkMode(activity);
        } else {
            SkinStatusBarUtils.setStatusBarLightMode(activity);
        }
    }

    public static void updateStatusBarColor(Activity activity) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			activity.getWindow().setStatusBarColor(SkinCompatResources.getColor(activity, R.color.status_bg_color));
		}
		updateStatusTextColor(activity);
    }
    ...
    public static void setStateBarTransparent(Activity activity) {
        SkinStatusBarUtils.translucent(activity);
    }
}