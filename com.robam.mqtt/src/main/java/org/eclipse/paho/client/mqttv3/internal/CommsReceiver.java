...
public class CommsReceiver implements Runnable {
    ...
    public void start(String threadName) {
        ...
        //@TRACE 855=starting
        if (log != null) log.fine(CLASS_NAME, methodName, "855");
    ...
    public void stop() {
        ...
        synchronized (lifecycle) {
            //@TRACE 850=stopping
            if (log != null) log.fine(CLASS_NAME, methodName, "850");
            if (running) {
                running = false;
                receiving = false;
        ...
        recThread = null;
        //@TRACE 851=stopped
        if (log != null) log.fine(CLASS_NAME, methodName, "851");
    }
    ...
    public void run() {
        ...
        while (running && (in != null)) {
            Log.i("2018022", "----------> receiver 进程：" + Process.myPid() + "  线程：" + Thread.currentThread().getName() + "  " +
                    Thread.currentThread().getId() + "  " + Thread.currentThread().toString());
            try {
                //@TRACE 852=network read message
                if (log != null) log.fine(CLASS_NAME, methodName, "852");
                ...
                if (log != null) log.fine(CLASS_NAME, methodName, "856", null, ex);
                running = false;
                // Token maybe null but that is handled in shutdown
                clientComms.shutdownConnection(token, ex);
            } catch (IOException ioe) {
                //@TRACE 853=Stopping due to IOException
                if (log != null) log.fine(CLASS_NAME, methodName, "853");
        ...
        //@TRACE 854=<
        if (log != null) log.fine(CLASS_NAME, methodName, "854");
    }
