...
public class Token {
	...
	public Token(String logContext) {
		if (log != null) log.setResourceName(logContext);
	}
	...
	public void waitForCompletion(long timeout) throws MqttException {
		...
		if (log != null) log.fine(CLASS_NAME,methodName, "407",new Object[]{getKey(), new Long(timeout), this});

		MqttWireMessage resp = waitForResponse(timeout);
		if (resp == null && !completed) {
			//@TRACE 406=key={0} timed out token={1}
			if (log != null) log.fine(CLASS_NAME,methodName, "406",new Object[]{getKey(), this});
	...
	protected MqttWireMessage waitForResponse(long timeout) throws MqttException {
		...
		synchronized (responseLock) {
			...
			if (log != null) log.fine(CLASS_NAME, methodName, "400",new Object[]{getKey(), new Long(timeout),new Boolean(sent),new Boolean(completed),(exception==null)?"false":"true",response,this},exception);

			while (!this.completed) {
				if (this.exception == null) {
					try {
						//@TRACE 408=key={0} wait max={1}
						if (log != null) log.fine(CLASS_NAME,methodName,"408",new Object[] {getKey(),new Long(timeout)});
				...
				if (!this.completed) {
					if (this.exception != null) {
						...
						if (log != null) log.fine(CLASS_NAME,methodName,"401",null,exception);
						throw exception;
					}
		...
		if (log != null) log.fine(CLASS_NAME,methodName, "402",new Object[]{getKey(), this.response});
	...
	protected void markComplete(MqttWireMessage msg, MqttException ex) {
		...
		if (log != null) log.fine(CLASS_NAME,methodName,"404",new Object[]{getKey(),msg,ex});
	 ...
	 */
		protected void notifyComplete() {
			...
			if (log != null) log.fine(CLASS_NAME,methodName,"404",new Object[]{getKey(),this.response, this.exception});
...
//		if (log != null) log.fine(CLASS_NAME,methodName, "405",new Object[]{this,this.exception});
//		synchronized (responseLock) {
//			responseLock.notifyAll();
//		}
//		synchronized (sentLock) {
...
//	}
		...
		synchronized (sentLock) {
			...
			while (!sent) {
				try {
					...
					if (log != null) log.fine(CLASS_NAME,methodName, "409",new Object[]{getKey()});
	...
	protected void notifySent() {
		...
		//@TRACE 403=> key={0}
		if (log != null) log.fine(CLASS_NAME, methodName, "403",new Object[]{getKey()});
		synchronized (responseLock) {
			this.response = null;
	...
	public void reset() throws MqttException {
		...
		//@TRACE 410=> key={0}
		if (log != null) log.fine(CLASS_NAME, methodName, "410",new Object[]{getKey()});
