...
public class CommsTokenStore {
	...
	public CommsTokenStore(String logContext) {
		...
		//@TRACE 308=<>
		if (log != null) log.fine(CLASS_NAME,methodName,"308");//,new Object[]{message});
	...
	public MqttToken removeToken(String key) {
		...
		//@TRACE 306=key={0}
		if (log != null) log.fine(CLASS_NAME,methodName,"306",new Object[]{key});
	...
	protected MqttDeliveryToken restoreToken(MqttPublish message) {
		...
		synchronized(tokens) {
			...
			if (this.tokens.containsKey(key)) {
				...
				if (log != null) log.fine(CLASS_NAME,methodName, "302",new Object[]{key, message,token});
			} else {
				...
				if (log != null) log.fine(CLASS_NAME,methodName,"303",new Object[]{key, message, token});
			}
		}
		return token;
	}
	...
	protected void saveToken(MqttToken token, MqttWireMessage message) throws MqttException {
		...
		synchronized(tokens) {
			if (closedResponse == null) {
				...
				if (log != null) log.fine(CLASS_NAME,methodName,"300",new Object[]{key, message});
	...
	protected void saveToken(MqttToken token, String key) {
		...
		synchronized(tokens) {
			...
			if (log != null) log.fine(CLASS_NAME,methodName,"307",new Object[]{key,token.toString()});
			token.internalTok.setKey(key);
			this.tokens.put(key, token);
		}
	}
	...
	protected void quiesce(MqttException quiesceResponse) {
		...
		synchronized(tokens) {
			//@TRACE 309=resp={0}
			if (log != null) log.fine(CLASS_NAME,methodName,"309",new Object[]{quiesceResponse});
	...
	public void open() {
		...
		synchronized(tokens) {
			//@TRACE 310=>
			if (log != null) log.fine(CLASS_NAME,methodName,"310");
	...
	public MqttDeliveryToken[] getOutstandingDelTokens() {
		...
		synchronized(tokens) {
			//@TRACE 311=>
			if (log != null) log.fine(CLASS_NAME,methodName,"311");
	...
	public Vector getOutstandingTokens() {
		...
		synchronized(tokens) {
			//@TRACE 312=>
			if (log != null) log.fine(CLASS_NAME,methodName,"312");
	...
	public void clear() {
		...
		//@TRACE 305=> {0} tokens
		if (log != null) log.fine(CLASS_NAME, methodName, "305", new Object[] {new Integer(tokens.size())});
		synchronized(tokens) {
			tokens.clear();
		}
	}
