...
public class CommsCallback implements Runnable {
	...
	CommsCallback(ClientComms clientComms) {
		...
		if (log != null) log.setResourceName(clientComms.getClient().getClientId());
	}
	...
	public void stop() {
		...
		synchronized (lifecycle) {
			if (running) {
				// @TRACE 700=stopping
				if (log != null) log.fine(CLASS_NAME, methodName, "700");
				running = false;
				if (!Thread.currentThread().equals(callbackThread)) {
					try {
						synchronized (workAvailable) {
							...
							// to finish
							if (log != null) log.fine(CLASS_NAME, methodName, "701");
			...
			callbackThread = null;
			// @TRACE 703=stopped
			if (log != null) log.fine(CLASS_NAME, methodName, "703");
		}
	}
	...
	public void run() {
		...
		while (running) {
			try {
				...
				try {
					synchronized (workAvailable) {
						if (running && messageQueue.isEmpty()
							...
							if (log != null) log.fine(CLASS_NAME, methodName, "704");
							workAvailable.wait();
						}
					}
				...
				if (log != null) log.fine(CLASS_NAME, methodName, "714", null, ex);
				running = false;
			    ...
			    synchronized (spaceAvailable) {
                    ...
                    if (log != null) log.fine(CLASS_NAME, methodName, "706");
	...
	private void handleActionComplete(MqttToken token)
		...
		synchronized (token) {
			...
			if (log != null) log.fine(CLASS_NAME, methodName, "705",	new Object[] { token.internalTok.getKey() });
	...
	public void connectionLost(MqttException cause) {
		...
		try {
			if (mqttCallback != null && cause != null) {
				...
				if (log != null) log.fine(CLASS_NAME, methodName, "708", new Object[] { cause });

				mqttCallback.connectionLost(cause);
			}
		} catch (Throwable t) {
			// Just if (log != null) log the fact that a throwable has caught connection lost
			// is called during shutdown processing so no need to do anything else
			// @TRACE 720=exception from connectionLost {0}
			if (log != null) log.fine(CLASS_NAME, methodName, "720", new Object[] { t });
		}
	}
		...
		if (token != null) {
			...
			if (asyncCB != null) {
				if (token.getException() == null) {
					...
					if (log != null) log.fine(CLASS_NAME, methodName, "716",
		...
		if (mqttCallback != null) {
			...
			synchronized (spaceAvailable) {
				while (running && !quiescing && messageQueue.size() >= INBOUND_QUEUE_SIZE) {
					try {
						...
						if (log != null) log.fine(CLASS_NAME, methodName, "709");
			...
			if (!quiescing) {
				...
				synchronized (workAvailable) {
					...
					if (log != null) log.fine(CLASS_NAME, methodName, "710");
					workAvailable.notifyAll();
				}
			}
		}
	...
	public void quiesce() {
		...
		synchronized (spaceAvailable) {
			...
			if (log != null) log.fine(CLASS_NAME, methodName, "711");
	...
	private void handleMessage(MqttPublish publishMessage)
		...
		if (mqttCallback != null) {
			...
			if (log != null) log.fine(CLASS_NAME, methodName, "713", new Object[] {
	...
	public void asyncOperationComplete(MqttToken token) {
		...
		if (running) {
			...
			synchronized (workAvailable) {
				...
				if (log != null) log.fine(CLASS_NAME, methodName, "715", new Object[] { token.internalTok.getKey() });
				workAvailable.notifyAll();
			}
		} else {
			...
			try {
				...
				if (log != null) log.fine(CLASS_NAME, methodName, "719", null, ex);
