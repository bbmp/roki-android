...
public class ClientState {
	...
	protected void restoreState() throws MqttException {
		...
		while(messageKeys.hasMoreElements()) {
			...
			if (log != null) log.fine(CLASS_NAME,methodName, "609", new Object[]{key});
	...
	private void restoreInflightMessages() {
		...
		while (keys.hasMoreElements()) {
			...
			if (msg instanceof MqttPublish) {
				...
				if (log != null) log.fine(CLASS_NAME,methodName, "610", new Object[]{key});
				...
				if (log != null) log.fine(CLASS_NAME,methodName, "611", new Object[]{key});
		...
		while (keys.hasMoreElements()) {
			...
			if (log != null) log.fine(CLASS_NAME,methodName, "612", new Object[]{key});
		...
		if (message instanceof MqttPublish) {
			synchronized (queueLock) {
				if (actualInFlight >= this.maxInflight) {
					...
					if (log != null) log.fine(CLASS_NAME, methodName, "613", new Object[]{new Integer(actualInFlight)});
				...
				if (log != null) log.fine(CLASS_NAME,methodName,"628", new Object[]{new Integer(message.getMessageId()), new Integer(innerMessage.getQos()), message});
			...
			if (log != null) log.fine(CLASS_NAME,methodName,"615", new Object[]{new Integer(message.getMessageId()), message});
	...
	protected void undo(MqttPublish message) throws MqttPersistenceException {
		...
		synchronized (queueLock) {
			...
			if (log != null) log.fine(CLASS_NAME,methodName,"618", new Object[]{new Integer(message.getMessageId()), new Integer(message.getMessage().getQos())});
	...
	public MqttToken checkForActivity() throws MqttException {
		...
		if (log != null) log.fine(CLASS_NAME,methodName,"616", new Object[]{});
		...
		if (connected && this.keepAlive > 0) {
            ...
            synchronized (pingOutstandingLock) {
                ...
                if (pingOutstanding > 0 && (time - lastInboundActivity >= keepAlive + delta)) {
                    ...
                    if (log != null) log.severe(CLASS_NAME,methodName,"619", new Object[]{new Long(this.keepAlive),new Long(lastOutboundActivity),new Long(lastInboundActivity), new Long(time), new Long(lastPing)});
                ...
                if (pingOutstanding == 0 && (time - lastOutboundActivity >= 2*keepAlive)) {
                	...
                	if (log != null) log.severe(CLASS_NAME,methodName,"642", new Object[]{new Long(this.keepAlive),new Long(lastOutboundActivity),new Long(lastInboundActivity), new Long(time), new Long(lastPing)});
                ...
                if ((pingOutstanding == 0 && (time - lastInboundActivity >= keepAlive - delta)) ||
                    ...
                    if (log != null) log.fine(CLASS_NAME,methodName,"620", new Object[]{new Long(this.keepAlive),new Long(lastOutboundActivity),new Long(lastInboundActivity)});
                    ...
                    if (log != null) log.fine(CLASS_NAME, methodName, "634", null);
                    nextPingTime = Math.max(1, getKeepAlive() - (time - lastOutboundActivity));
                }
            }
            //@TRACE 624=Schedule next ping at {0}                                                                                                                                                                                
            if (log != null) log.fine(CLASS_NAME, methodName,"624", new Object[]{new Long(nextPingTime)});
			...
			while (result == null) {
				...
				if ((pendingMessages.isEmpty() && pendingFlows.isEmpty()) || 
					...
					try {
						...
						if (log != null) log.fine(CLASS_NAME,methodName, "644");						
 
						queueLock.wait();
						
						//@TRACE 647=new work or ping arrived 
						if (log != null) log.fine(CLASS_NAME,methodName, "647");
				...
				if (!connected && 
					...
					if (log != null) log.fine(CLASS_NAME,methodName,"621");
...
//				checkForActivity(); //Use pinger, don't check here
				...
				if (!pendingFlows.isEmpty()) {
					...
					if (result instanceof MqttPubRel) {
						...
						if (log != null) log.fine(CLASS_NAME,methodName,"617", new Object[]{new Integer(inFlightPubRels)});
					}
					...
					if (actualInFlight < this.maxInflight) {
						...
						if (log != null) log.fine(CLASS_NAME,methodName,"623",new Object[]{new Integer(actualInFlight)});
					} else {
						//@TRACE 622=inflight window full
						if (log != null) log.fine(CLASS_NAME,methodName,"622");				
					}
				}			
			}
		}
    ...
    public void notifySentBytes(int sentBytesCount) {
        ...
        if (log != null) log.fine(CLASS_NAME, methodName, "631", new Object[] {
	...
	protected void notifySent(MqttWireMessage message) {
		...
		this.lastOutboundActivity = System.currentTimeMillis();
		//@TRACE 625=key={0}
		if (log != null) log.fine(CLASS_NAME,methodName,"625",new Object[]{message.getKey()});
        ...
        if (message instanceof MqttPingReq) {
            synchronized (pingOutstandingLock) {
                ...
                if (log != null) log.fine(CLASS_NAME,methodName,"635",new Object[]{ new Integer(pingOutstanding)});
            }
        }
	...
	private void decrementInFlight() {
		...
		synchronized (queueLock) {
			...
			if (log != null) log.fine(CLASS_NAME,methodName,"646",new Object[]{new Integer(actualInFlight)});
...
//		if (quiescing && actualInFlight == 0 && pendingFlows.size() == 0 && inFlightPubRels == 0 && callback.isQuiesced()) {
		...
		if (quiescing && tokC == 0 && pendingFlows.size() == 0 && callback.isQuiesced()) {
			...
			if (log != null) log.fine(CLASS_NAME,methodName,"626",new Object[]{new Boolean(quiescing), new Integer(actualInFlight), new Integer(pendingFlows.size()), new Integer(inFlightPubRels), Boolean.valueOf(callback.isQuiesced()), new Integer(tokC)});
			synchronized (quiesceLock) {
    ...
    public void notifyReceivedBytes(int receivedBytesCount) {
        ...
        if (log != null) log.fine(CLASS_NAME, methodName, "630", new Object[] {
	...
	protected void notifyReceivedAck(MqttAck ack) throws MqttException {
		...
		if (log != null) log.fine(CLASS_NAME, methodName, "627", new Object[] {
		...
		if (ack instanceof MqttPubRec) {
            ...
            if (log != null) log.fine(CLASS_NAME,methodName,"636",new Object[]{ new Integer(pingOutstanding)});
	...
	protected void notifyReceivedMsg(MqttWireMessage message) throws MqttException {
		...
		if (log != null) log.fine(CLASS_NAME, methodName, "651", new Object[] {
	...
	protected void notifyComplete(MqttToken token) throws MqttException {
		...
		if (message != null && message instanceof MqttAck) {
			...
			if (log != null) log.fine(CLASS_NAME, methodName, "629", new Object[] {
			...
			if (ack instanceof MqttPubAck) {
				...
				if (log != null) log.fine(CLASS_NAME, methodName, "650",
				...
				if (log != null) log.fine(CLASS_NAME, methodName, "645", new Object[] {
	...
	protected void notifyResult(MqttWireMessage ack, MqttToken token, MqttException ex) {
		...
		if (ack != null && ack instanceof MqttAck && !(ack instanceof MqttPubRec)) {
			...
			if (log != null) log.fine(CLASS_NAME,methodName, "648", new Object [] {token.internalTok.getKey(), ack, ex});
			callback.asyncOperationComplete(token);
		}
		...
		if (ack == null ) {
			...
			if (log != null) log.fine(CLASS_NAME,methodName, "649", new Object [] { token.internalTok.getKey(), ex});
			callback.asyncOperationComplete(token);
		}
	}
	...
	public void connected() {
		...
		//@TRACE 631=connected
		if (log != null) log.fine(CLASS_NAME, methodName, "631");
		this.connected = true;
	...
	public Vector resolveOldTokens(MqttException reason) {
		...
		//@TRACE 632=reason {0}
		if (log != null) log.fine(CLASS_NAME,methodName,"632", new Object[] {reason});
	...
	public void disconnected(MqttException reason) {
		...
		if (log != null) log.fine(CLASS_NAME,methodName,"633", new Object[] {reason});		
	...
	public void quiesce(long timeout) {
		...
		if (timeout > 0 ) {
			//@TRACE 637=timeout={0}
			if (log != null) log.fine(CLASS_NAME,methodName, "637",new Object[]{new Long(timeout)});
			synchronized (queueLock) {
				this.quiescing = true;
			}
			...
			synchronized (quiesceLock) {
				try {			
					...
					if (tokc > 0 || pendingFlows.size() >0 || !callback.isQuiesced()) {
						...
						if (log != null) log.fine(CLASS_NAME, methodName,"639", new Object[]{new Integer(actualInFlight), new Integer(pendingFlows.size()), new Integer(inFlightPubRels), new Integer(tokc)});
			...
			//@TRACE 640=finished
			if (log != null) log.fine(CLASS_NAME, methodName, "640");
		}
	}
	...
	public void notifyQueueLock() {
		...
		synchronized (queueLock) {
			...
			if (log != null) log.fine(CLASS_NAME,methodName,"638");
			queueLock.notifyAll();
		}
	}
	...
	protected void deliveryComplete(MqttPublish message) throws MqttPersistenceException {
		...
		if (log != null) log.fine(CLASS_NAME,methodName,"641", new Object[]{new Integer(message.getMessageId())});
