...
public class CommsSender implements Runnable {
	...
	public void stop() {
		...
		synchronized (lifecycle) {
			...
			if (log != null) log.fine(CLASS_NAME,methodName,"800");
			if (running) {
				running = false;
			...
			sendThread=null;
			//@TRACE 801=stopped
			if (log != null) log.fine(CLASS_NAME,methodName,"801");
		}
	}
	...
	public void run() {
		...
		while (running && (out != null)) {
			try {
				...
				if (message != null) {
					...
					if (log != null) log.fine(CLASS_NAME,methodName,"802", new Object[] {message.getKey(),message});
					...
					if (log != null) log.fine(CLASS_NAME,methodName,"803");
		...
		//@TRACE 805=<
		if (log != null) log.fine(CLASS_NAME, methodName,"805");
